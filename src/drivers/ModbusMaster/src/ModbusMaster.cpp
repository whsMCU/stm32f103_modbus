...
@ingroup setup
*/
void ModbusMaster::begin(uint8_t serial, uint8_t slave, uint8_t de_pin)
{
...
//  for (i = 0; i < u8ModbusADUSize; i++)
//  {
	  uartWriteIT(_serial, &u8ModbusADU[i], u8ModbusADUSize);// _serial->write(u8ModbusADU[i]);
//  }
  
  u8ModbusADUSize = 0;
  uartFlush(_serial);//_serial->flush();    // flush transmit buffer
//  if (_postTransmission)
//  {
//    _postTransmission();
//  }
  gpioPinWrite(_de_pin, _DEF_LOW);
  
  // loop until we run out of time or bytes, or an error occurs
  u32StartTime = millis();
  while (u8BytesLeft && !u8MBStatus)
  {
    if (uartAvailable(_serial))//(_serial->available())
    {
      u8ModbusADU[u8ModbusADUSize++] = uartRead(_serial);//_serial->read();
      u8BytesLeft--;
    }
    else
    {
